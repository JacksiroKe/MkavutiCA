...
Begin VB.UserControl jsMdiTab 
   ...
   Picture         =   "jsMdiTab.ctx":0000
   ScaleHeight     =   150
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   253
   ToolboxBitmap   =   "jsMdiTab.ctx":0C42
   Begin VB.PictureBox picDefault 
      Height          =   255
      Left            =   1560
      Picture         =   "jsMdiTab.ctx":0F54
      ScaleHeight     =   195
      ScaleWidth      =   225
      TabIndex        =   0
      Top             =   840
...
'****************************************************************************
'*  Title:        jsMdiTab ver 2.0 (ActiveX Control)                       *
'*  Function:     Modern Tabbed MDI interface                               *
'*  Author:       Juned Chhipa                                              *
'*  Created:      February 2009                                             *
'*  Contact:      juned.chhipa@yahoo.com                                    *                                                                        *
'*  Copyright © 2009 Juned Chhipa. All Rights Reserved                      *
'****************************************************************************
'*  Title:        Original RevMDITabs (ActiveX Control)                     *
'*  Function:     Modern Tabbed MDI interface                               *
'*  Author:       Andrea Batina                                             *
'*  Created:      November 2004                                             *
'*  Copyright © 2004 Andrea Batina. All Rights Reserved                     *
'*  Downloaded from                                                         *
'*  http://www.pscode.com/vb/scripts/ShowCode.asp?txtCodeId=57301&lngWId=1  *
'****************************************************************************
'* jsMdiTab is an ActiveX control that allows you to quickly provide       *
'* Office XP, .NET,  OneNote and Whidbey style                              *
'* tabbed MDI interfaces in your VB applications.                           *
'*                                                                          *
'* Originally the code was written by Andrea Batina                         *
'* CREDITS goes to Paul Caton and Andrea Batina ;-)                         *
'* Send your suggestions, feedback, bug reports to juned.chhipa@yahoo.com   *
'****************************************************************************
...
#If False Then
    Private mtsdotNET_1, mtsdotNET_2, mtsOffice2003, mtsOffice2000, mstOneNote, mtsWhidbey
#End If
...
#If False Then
    Private ensScrollButtons, ensDropDownButton
#End If
...
Public Event DropdownButtonClick() '(x As Long, y As Long)
...
Private m_hWndExcludeChild      As Collection
Private m_tTabR()               As RECT                 ' Tab items dimensions
Private m_tCloseBtnR()          As RECT
...
Private m_tDropBtnRect          As RECT                 ' Dropdown button rect
...
Private m_lColorTable(1 To 8)   As Long                 ' OneNote style colors
...
Private m_lClrTabBack2          As Long
...
Private m_lClrSelButtons        As Long                 ' Bg color for close, scroll butons
...
Private m_NavigationStyle       As enumNavigationStyle
...
Private Sub pDrawControl(ByVal lHwnd As Long)
    ...
    If m_oMemDC Is Nothing Then
        Exit Sub
    End If

    Select Case m_eStyle
    Case mtsdotNET_1, mtsdotNET_2
        DrawTabsdotNET lHwnd
    Case mtsOffice2000
        DrawTabsOffice2000 lHwnd
    Case mstOneNote
        DrawOneNoteStyle lHwnd
    Case mtsWhidbey
        DrawTabsWhidbey lHwnd
    Case mtsOffice2003
        DrawTabsOffice2003 lHwnd
    End Select

End Sub

Private Sub DrawTabsWhidbey(ByVal lHwnd As Long)
    ...
    With m_oMemDC
        ...
        DeleteObject lPen
        '.DrawLine tR.Left, tR.Bottom - 2, tR.Left, m_lTabHeight - 3, .TranslateColor(vbButtonShadow)
        lPen = CreatePen(PS_SOLID, 1, .TranslateColor(vbButtonShadow))
        lPenOld = SelectObject(lChDC, lPen)
        ...
        '============================================================
        ' Draw bar background
        .FillRect tR.Left, tR.Top, tR.Right, m_lTabHeight - 3, vbButtonFace
        ' Draw bar highlight line
        .DrawLine tR.Left, m_lTabHeight - 3, tR.Right - 1, m_lTabHeight - 3, .TranslateColor(vbButtonShadow)
        ...
        ' left
        .DrawLine tR.Left, tR.Top, tR.Left, tR.Bottom, vbButtonShadow
        '== END BORDERS
        '============================================================
    End With
    ...
    If m_hWndChild.Count > 0 Then
        With m_oMemDCTabs
            ...
            For Each hWndChild In m_hWndChild
                ...
                m_TabCaption = CaptionW(hWndChild)
                ...
                m_tTabR(i).Right = m_tTabR(i).Left + .TextWidth(m_TabCaption) + 12
            ...
            If pIsPrevButtonEnabled Then
                If m_tTabR(m_hWndChild.Count).Right - m_lOffsetX < tR.Right - 49 Then
                    pScrollPrev
                End If
            End If
            ...
            .FillRect 0, 0, .Width, m_lTabHeight - 3, .TranslateColor(vbButtonFace)
            ' Draw highlight line
            ...
            For Each hWndChild In m_hWndChild
                ...
                If bIntersect Then
                    ...
                    If i = 1 Or lSelMDIChild = hWndChild Then
                        ...
                        If lSelMDIChild = hWndChild Then
                            ' -selected tab
                            .SetPixel m_tTabR(i).Left - 15, m_tTabR(i).Bottom - 2, .TranslateColor(&HB99D7F)
                            .DrawLine m_tTabR(i).Left - 14, m_tTabR(i).Bottom - 2, m_tTabR(i).Left + 2, m_tTabR(i).Top + 2, &HB99D7F
                            .DrawLine m_tTabR(i).Left + 2, m_tTabR(i).Top + 2, m_tTabR(i).Left + 6, m_tTabR(i).Top + 1, &HB99D7F
                            .DrawLine m_tTabR(i).Left + 6, m_tTabR(i).Top, m_tTabR(i).Right - 2, m_tTabR(i).Top, &HB99D7F
                            .SetPixel m_tTabR(i).Right - 2, m_tTabR(i).Top + 1, .TranslateColor(&HB99D7F)
                            .DrawLine m_tTabR(i).Right - 1, m_tTabR(i).Top + 2, m_tTabR(i).Right - 1, m_tTabR(i).Bottom, &HB99D7F
                        Else
                            ' --first tab
                        ...
                        If lSelMDIChild = hWndChild Then
                            .DrawLine m_tTabR(i).Left - 15, m_tTabR(i).Bottom - 1, m_tTabR(i).Right, m_tTabR(i).Bottom - 1, vb3DHighlight
                        Else
                    ...
                    Else
                        ...
                        With m_tTabR(i)
                            ...
                            ' Draw tab border
                            m_oMemDCTabs.SetPixel .Left + 1, .Top + 3, m_oMemDCTabs.TranslateColor(vbButtonShadow)
                            m_oMemDCTabs.DrawLine .Left + 2, .Top + 2, .Left + 6, .Top + 1, vbButtonShadow
                            m_oMemDCTabs.DrawLine .Left + 6, .Top, .Right - 2, .Top, vbButtonShadow
                            m_oMemDCTabs.SetPixel .Right - 2, .Top + 1, m_oMemDCTabs.TranslateColor(vbButtonShadow)
                            m_oMemDCTabs.DrawLine .Right - 1, .Top + 2, .Right - 1, .Bottom, vbButtonShadow
                            ' Draw tab highlight line
                            m_oMemDCTabs.DrawLine .Left + 2, .Top + 3, .Left + 6, .Top + 2, vbButtonFace
                            m_oMemDCTabs.DrawLine .Left + 6, .Top + 1, .Right - 2, .Top + 1, vbButtonFace
                            m_oMemDCTabs.DrawLine .Right - 2, .Top + 2, .Right - 2, .Bottom - 1, vbButtonFace
                        End With
                    End If
                    ...
                    If lSelMDIChild = hWndChild Then
                        .FillRect tRgnBox.Left, tRgnBox.Top, tRgnBox.Right, tRgnBox.Bottom, .TranslateColor(vbWhite)
                    Else
                        .DrawGradient tRgnBox.Left, tRgnBox.Top, tRgnBox.Right, tRgnBox.Bottom, .TranslateColor(vbButtonFace), .TranslateColor(vbWhite), gdVertical
                    End If
                    SelectClipRgn .hDc, 0
                    DeleteObject lRgn
                    ...
                    ' Draw tab caption
                    lTabTop = m_tTabR(i).Top + (((m_tTabR(i).Bottom - m_tTabR(i).Top) / 2) - (.TextHeight(m_TabCaption) / 2))
                    If m_bDrawIcons Then
                        .DrawText m_TabCaption, m_tTabR(i).Left + 20, lTabTop, m_tTabR(i).Right, m_tTabR(i).Bottom, DT_CENTER
                        .DrawPicture pGetSmallIcon(CLng(hWndChild)), m_tTabR(i).Left + 5, m_tTabR(i).Top + 3
                    Else
                        .DrawText m_TabCaption, m_tTabR(i).Left, lTabTop, m_tTabR(i).Right, m_tTabR(i).Bottom, DT_CENTER
                    End If
                End If
            Next
        End With
        ...
        With m_oMemDC
            ...
            '.DrawGradient tT.Left, tT.Top, tT.Right, tT.Bottom, .TranslateColor(vbButtonFace), .TranslateColor(vbButtonFace), gdVertical
            ...
            .DrawLine tT.Right - 1, tT.Top, tT.Right - 1, tT.Bottom, vbButtonShadow
            If m_NavigationStyle = ensDropDownButton Then
                m_lButtonsSize = 7 - lBtnOffset - 26
            Else
                m_lButtonsSize = 7 - lBtnOffset
            End If
            ' Draw close button
            With m_tCloseBtnRect
                .Left = tT.Right - (19 - lBtnOffset)
                .Top = 5
                .Right = m_tCloseBtnRect.Left + 14
                .Bottom = m_tCloseBtnRect.Top + 15
            ...
            If m_NavigationStyle = ensScrollButtons Then
                ' Draw button border
                If m_lSelBtn = 2 Then
                    ' If it is selected
            ...
            If m_NavigationStyle = ensDropDownButton Then
                ' --Dropdown button
                m_tDropBtnRect.Left = m_tNextBtnRect.Left
                m_tDropBtnRect.Top = 5
                m_tDropBtnRect.Right = m_tNextBtnRect.Right
                m_tDropBtnRect.Bottom = m_tNextBtnRect.Top + 15
                m_lButtonsSize = m_lButtonsSize + 14
                LSet tT = m_tDropBtnRect
                ' Draw X sign using Marlett font
                ...
                If m_lSelBtn = 4 Then
                    ' If it is pressed
                    If m_lPressedBtn = 4 Then
                        .FillRect m_tDropBtnRect.Left, m_tDropBtnRect.Top, m_tDropBtnRect.Right, m_tDropBtnRect.Bottom, .AlphaBlend(vbHighlight, .AlphaBlend(vbWindowBackground, vbButtonFace, 220), 85)
                        .Draw3DRect m_tDropBtnRect.Left, m_tDropBtnRect.Top, m_tDropBtnRect.Right, m_tDropBtnRect.Bottom, vbHighlight, vbHighlight
                        ' If it is selected
                    Else
                        .FillRect m_tDropBtnRect.Left, m_tDropBtnRect.Top, m_tDropBtnRect.Right, m_tDropBtnRect.Bottom, .AlphaBlend(vbHighlight, .AlphaBlend(vbWindowBackground, vbButtonFace, 220), 85)
                        .Draw3DRect m_tDropBtnRect.Left, m_tDropBtnRect.Top, m_tDropBtnRect.Right, m_tDropBtnRect.Bottom, vbHighlight, vbHighlight
                ...
                If m_lPressedBtn = 4 Then
                    ' Draw X sign
                    .DrawText "6", m_tDropBtnRect.Left + 1, m_tDropBtnRect.Top + 1, m_tDropBtnRect.Right, m_tDropBtnRect.Bottom
                Else
                    ' Draw X sign
                    .DrawText "6", m_tDropBtnRect.Left, m_tDropBtnRect.Top, m_tDropBtnRect.Right, m_tDropBtnRect.Bottom
                End If
            End If
            ' Draw topmost border
            .DrawLine tR.Left, tR.Top, tR.Right, tR.Top, vbButtonShadow
            '============================================================
        End With
    Else
        With m_oMemDC
...
Private Sub DrawOneNoteStyle(ByVal lHwnd As Long)
    ...
    With m_oMemDC
        ...
        '============================================================
        ' Draw bar background
        .DrawGradient tR.Left, tR.Top, tR.Right, m_lTabHeight - 3, .LightenColor(.TranslateColor(vbButtonFace), .TranslateColor(vbWindowBackground), 205), .TranslateColor(vbButtonFace), gdVertical
        ' Draw bar highlight line
        .DrawLine tR.Left, m_lTabHeight - 3, tR.Right - 1, m_lTabHeight - 3, .TranslateColor(vbButtonShadow)
        ' Draw tab highight line
        .DrawLine tR.Left + 1, m_lTabHeight - 2, tR.Right - 1, m_lTabHeight - 2, lHighlightClr
    ...
    If m_hWndChild.Count > 0 Then
        With m_oMemDCTabs
            ...
            ' Draw control background
            .DrawGradient 0, 0, .Width, m_lTabHeight - 3, .LightenColor(.TranslateColor(vbButtonFace), .TranslateColor(vbWindowBackground), 205), .TranslateColor(vbButtonFace), gdVertical
            ' Draw highlight line
            .DrawLine 0, m_lTabHeight - 3, .Width, m_lTabHeight - 3, vbButtonShadow
            .DrawLine 0, m_lTabHeight - 2, .Width, m_lTabHeight - 2, lHighlightClr
            .DrawLine 0, m_lTabHeight - 1, .Width, m_lTabHeight - 1, vbButtonShadow
                ...
                If bIntersect Then
                    ...
                    Else
                        ...
                        With m_tTabR(i)
                            ...
                            m_oMemDCTabs.DrawLine .Left + 2, .Top + 3, .Left + 6, .Top + 2, vbButtonFace
                            m_oMemDCTabs.DrawLine .Left + 6, .Top + 1, .Right - 2, .Top + 1, vbButtonFace
                            m_oMemDCTabs.DrawLine .Right - 2, .Top + 2, .Right - 2, .Bottom, vbButtonFace
                        End With
                    End If
                    ...
                    SelectClipRgn .hDc, lRgn
                    GetRgnBox lRgn, tRgnBox
                    .DrawGradient tRgnBox.Left, tRgnBox.Top, tRgnBox.Right, tRgnBox.Bottom, m_lColorTable(m_oColorIndex(i)), .LightenColor(m_lColorTable(m_oColorIndex(i)), .TranslateColor(vbWhite), 240), gdVertical
                    SelectClipRgn .hDc, 0
                    DeleteObject lRgn
        ...
        With m_oMemDC
            ...
            tT.Top = tR.Top '+ 1
            tT.Bottom = m_lTabHeight - 2
            .DrawGradient tT.Left, tT.Top, tT.Right, tT.Bottom, .LightenColor(.TranslateColor(vbButtonFace), .TranslateColor(vbWindowBackground), 205), .TranslateColor(vbButtonFace), gdVertical
            .DrawLine tT.Left, tT.Bottom - 1, tT.Right, tT.Bottom - 1, vbButtonShadow
            If m_NavigationStyle = ensDropDownButton Then
                m_lButtonsSize = 7 - lBtnOffset - 26
            Else
...
Private Sub DrawTabsOffice2000(ByVal lHwnd As Long)
        ...
        With m_oMemDCTabs
            ...
            For Each hWndChild In m_hWndChild
                ...
                If bIntersect Then
                    ...
                    lTabTop = m_tTabR(i).Top + (((m_tTabR(i).Bottom - m_tTabR(i).Top) / 2) - (.TextHeight(m_TabCaption) / 2))
                    If m_bDrawIcons Then
                        .DrawText m_TabCaption, m_tTabR(i).Left + 20, lTabTop + lTextTop, m_tTabR(i).Right, m_tTabR(i).Bottom, DT_CENTER
                        .DrawPicture pGetSmallIcon(CLng(hWndChild)), m_tTabR(i).Left + 5, m_tTabR(i).Top + 3
                    Else
                        .DrawText m_TabCaption, m_tTabR(i).Left, lTabTop + lTextTop, m_tTabR(i).Right, m_tTabR(i).Bottom, DT_CENTER
                    End If
                End If
            Next
        End With
        ...
        With m_oMemDC
            ...
            If m_NavigationStyle = ensDropDownButton Then
                ...
                If m_lSelBtn = 4 Then
                    ' If it is pressed
                    If m_lPressedBtn = 4 Then
                        .Draw3DRect m_tDropBtnRect.Left, m_tDropBtnRect.Top, m_tDropBtnRect.Right, m_tDropBtnRect.Bottom, vbButtonShadow, m_lClrButtonBorder
                        ' If it is selected
                    Else
                        .Draw3DRect m_tDropBtnRect.Left, m_tDropBtnRect.Top, m_tDropBtnRect.Right, m_tDropBtnRect.Bottom, m_lClrButtonBorder, vbButtonShadow
                    End If
                End If
                ' If it is pressed
                If m_lPressedBtn = 4 Then
...
Private Sub DrawTabsOffice2003(ByVal lHwnd As Long)
...
Dim bIntersect As Long
    ...
    With m_oMemDC
        ...
        DeleteObject lPen
        ' Draw inner border line
        lPen = CreatePen(PS_SOLID, 1, .TranslateColor(.AlphaBlend(m_lClrback, vbWhite, 100)))
        lPenOld = SelectObject(lChDC, lPen)
        ...
        DeleteObject lPen
        ' Draw inner-inner border line
        lPen = CreatePen(PS_SOLID, 1, .TranslateColor(vbButtonFace))
        lPenOld = SelectObject(lChDC, lPen)
        ...
        '========================================
        ' Draw left border
        .DrawLine tR.Left, tR.Top, tR.Left, m_lTabHeight, m_lClrBorder
        .DrawLine tR.Left + 1, tR.Top + 1, tR.Left + 1, m_lTabHeight, .AlphaBlend(m_lClrback, vbWhite, 150)
        ' Draw top border
        .DrawLine tR.Left, tR.Top, tR.Right, tR.Top, m_lClrOuterBorder
        ' Draw right border
        .DrawLine tR.Right - 1, tR.Top, tR.Right - 1, m_lTabHeight, m_lClrBorder
        .DrawLine tR.Right - 2, tR.Top + 1, tR.Right - 2, m_lTabHeight, .AlphaBlend(m_lClrback, vbWhite, 150)
        '============================================================
        ' Draw bar background
        .DrawGradient tR.Left + 2, tR.Top, tR.Right - 2, m_lTabHeight - 3, m_lClrback, .AlphaBlend(m_lClrback, vbWhite, 20), gdVertical
        ' Draw tab highight line
        .DrawLine tR.Left + 1, m_lTabHeight - 2, tR.Right - 1, m_lTabHeight - 2, m_lClrback
        ' Draw bar highlight line
        .DrawLine tR.Left + 3, m_lTabHeight - 3, tR.Right - 3, m_lTabHeight - 3, m_lClrBorder
        .DrawLine tR.Left + 3, m_lTabHeight - 1, tR.Right - 3, m_lTabHeight - 1, m_lClrback
        .DrawLine tR.Left + 3, m_lTabHeight - 2, tR.Right - 3, m_lTabHeight - 2, m_lClrback
        '== END BORDERS
        '============================================================
    End With
    ...
    If m_hWndChild.Count > 0 Then
        With m_oMemDCTabs
            ...
            .DrawGradient 0, 0, .Width, m_lTabHeight - 3, m_lClrback, .AlphaBlend(m_lClrback, vbWhite, 20), gdVertical
            
            'Background line
            .DrawLine 0, m_tTabR(i).Bottom - 1, tR.Right, m_tTabR(i).Bottom - 1, m_lClrBorder
            ...
            For Each hWndChild In m_hWndChild
                ...
                If bIntersect Then
                    ...
                    If lSelMDIChild = hWndChild Then
                        ...
                        ' Draw tab gradient
                        ' -This is the actual tab
                        lRgn = CreateRoundRectRgn(m_tTabR(i).Left, m_tTabR(i).Top + 1, m_tTabR(i).Right, m_tTabR(i).Bottom + 1, 1, 1)
                        SelectClipRgn .hDc, lRgn
                        GetRgnBox lRgn, tRgnBox
                        ' --Draw Tab Gradient Here
                        .DrawGradient m_tTabR(i).Left, m_tTabR(i).Top, m_tTabR(i).Right, m_tTabR(i).Bottom, m_lClrTabBack2, m_lClrTabBack1, gdVertical
                        SelectClipRgn .hDc, 0
                        DeleteObject lRgn
                        
                        ' Draw tab border
                        .DrawLine m_tTabR(i).Left, m_tTabR(i).Top, m_tTabR(i).Left, m_tTabR(i).Bottom + 5, m_lClrBorder
                        .DrawLine m_tTabR(i).Left, m_tTabR(i).Top, m_tTabR(i).Right, m_tTabR(i).Top, m_lClrBorder
                        .DrawLine m_tTabR(i).Right - 1, m_tTabR(i).Top + 1, m_tTabR(i).Right - 1, m_tTabR(i).Bottom, m_lClrBorder
                        .SetPixel m_tTabR(i).Right - 1, m_tTabR(i).Top, .AlphaBlend(m_lClrBorder, vbButtonFace, 110)
                        .SetPixel m_tTabR(i).Right, m_tTabR(i).Top + 1, .AlphaBlend(vbHighlight, &HE0E0E0, 50)
                        
                        ' -Create shadow rgn
                        lRgn = CreateRoundRectRgn(m_tTabR(i).Right, m_tTabR(i).Top + 2, m_tTabR(i).Right + 3, m_tTabR(i).Bottom, 1, 1)
                        SelectClipRgn .hDc, lRgn
                        GetRgnBox lRgn, tRgnBox
                        ' -Shadow Gradient
                        .DrawGradient m_tTabR(i).Right, m_tTabR(i).Top + 2, m_tTabR(i).Right + 3, m_tTabR(i).Bottom, .AlphaBlend(vbButtonShadow, .AlphaBlend(vbGrayText, m_lClrTabBack2, 60), 160), .AlphaBlend(vbButtonShadow, .AlphaBlend(m_lClrback, &HE0E0E0, 20), 45), gdVertical
                        SelectClipRgn .hDc, 0
                        DeleteObject lRgn
                    ...
                    Else
                        ...
                        ' repair broken lines (lol)
                        .DrawLine m_tTabR(i).Left, m_tTabR(i).Bottom - 1, m_tTabR(m_hWndChild.Count).Right + 10, m_tTabR(i).Bottom - 1, m_lClrBorder
                    End If
                    ' Draw tab caption
                    lTabTop = m_tTabR(i).Top + (((m_tTabR(i).Bottom - m_tTabR(i).Top) / 2) - (.TextHeight(m_TabCaption) / 2))
                    If m_bDrawIcons Then
                        .DrawText m_TabCaption, m_tTabR(i).Left + 23, lTabTop, m_tTabR(i).Right, m_tTabR(i).Bottom, DT_CENTER
                        .DrawPicture pGetSmallIcon(CLng(hWndChild)), m_tTabR(i).Left + 5, m_tTabR(i).Top + 3
                    Else
                        .DrawText m_TabCaption, m_tTabR(i).Left, lTabTop, m_tTabR(i).Right, m_tTabR(i).Bottom, DT_CENTER
                    End If
        ...
        With m_oMemDC
            ...
            tT.Top = tR.Top '+ 1
            tT.Bottom = m_lTabHeight - 2
            .DrawGradient tT.Left, tT.Top, tT.Right, tT.Bottom, m_lClrback, .AlphaBlend(m_lClrback, vbWhite, 20), gdVertical
            m_lButtonsSize = 7 - lBtnOffset
            ' line under the next, prev and close btns
            .DrawLine tT.Left, tT.Bottom - 1, tT.Right - 2, tT.Bottom - 1, m_lClrBorder
            If m_NavigationStyle = ensDropDownButton Then
                m_lButtonsSize = 7 - lBtnOffset - 22
            Else
                m_lButtonsSize = 7 - lBtnOffset
            End If
            ' Draw close button
            ...
            If m_lSelBtn = 3 Then
                ' If it is pressed
                If m_lPressedBtn = 3 Then
                    .FillRect m_tCloseBtnRect.Left, m_tCloseBtnRect.Top, m_tCloseBtnRect.Right, m_tCloseBtnRect.Bottom, .AlphaBlend(vbBlack, m_lClrSelButtons, 40)
                    .Draw3DRect m_tCloseBtnRect.Left, m_tCloseBtnRect.Top, m_tCloseBtnRect.Right, m_tCloseBtnRect.Bottom, .AlphaBlend(vbBlack, vbHighlight, 120), .AlphaBlend(vbBlack, vbHighlight, 120)
                Else
                    .FillRect m_tCloseBtnRect.Left, m_tCloseBtnRect.Top, m_tCloseBtnRect.Right, m_tCloseBtnRect.Bottom, m_lClrSelButtons
                    .Draw3DRect m_tCloseBtnRect.Left, m_tCloseBtnRect.Top, m_tCloseBtnRect.Right, m_tCloseBtnRect.Bottom, .AlphaBlend(vbBlack, vbHighlight, 60), .AlphaBlend(vbBlack, vbHighlight, 60)
                End If
            End If
            ' Draw X sign
            .DrawText "r", m_tCloseBtnRect.Left + 3, m_tCloseBtnRect.Top + 3, m_tCloseBtnRect.Right, m_tCloseBtnRect.Bottom
            ...
            If m_NavigationStyle = ensScrollButtons Then
                ...
                If m_lSelBtn = 2 Then
                    ' If it is pressed
                    If m_lPressedBtn = 2 Then
                        .FillRect m_tNextBtnRect.Left, m_tNextBtnRect.Top, m_tNextBtnRect.Right, m_tNextBtnRect.Bottom, .AlphaBlend(vbBlack, m_lClrSelButtons, 40)
                        .Draw3DRect m_tNextBtnRect.Left, m_tNextBtnRect.Top, m_tNextBtnRect.Right, m_tNextBtnRect.Bottom, .AlphaBlend(vbBlack, vbHighlight, 120), .AlphaBlend(vbBlack, vbHighlight, 120)
                        ' If it is selected
                    Else
                        .FillRect m_tNextBtnRect.Left, m_tNextBtnRect.Top, m_tNextBtnRect.Right, m_tNextBtnRect.Bottom, m_lClrSelButtons
                        .Draw3DRect m_tNextBtnRect.Left, m_tNextBtnRect.Top, m_tNextBtnRect.Right, m_tNextBtnRect.Bottom, .AlphaBlend(vbBlack, vbHighlight, 60), .AlphaBlend(vbBlack, vbHighlight, 60)
                    End If
                End If
                tT.Left = tT.Left + 4
                tT.Top = tT.Top + 2
                ...
                If Not pIsNextButtonEnabled Then
                    ' Draw disabled arrow
                    For i = 0 To 4
                        .DrawLine tT.Left + i, tT.Top + i, tT.Left + i, tT.Bottom - i, .AlphaBlend(vbHighlight, vbGrayText, 90)
                    Next
                End If
            End If
            '========================================
            ...
            If m_NavigationStyle = ensScrollButtons Then
                ...
                If m_lSelBtn = 1 Then
                    ' If it is pressed
                    If m_lPressedBtn = 1 Then
                        .FillRect m_tPrevBtnRect.Left, m_tPrevBtnRect.Top, m_tPrevBtnRect.Right, m_tPrevBtnRect.Bottom, .AlphaBlend(vbBlack, m_lClrSelButtons, 40)
                        .Draw3DRect m_tPrevBtnRect.Left, m_tPrevBtnRect.Top, m_tPrevBtnRect.Right, m_tPrevBtnRect.Bottom, .AlphaBlend(vbBlack, vbHighlight, 120), .AlphaBlend(vbBlack, vbHighlight, 120)
                        ' If it is selected
                    Else
                        .FillRect m_tPrevBtnRect.Left, m_tPrevBtnRect.Top, m_tPrevBtnRect.Right, m_tPrevBtnRect.Bottom, m_lClrSelButtons
                        .Draw3DRect m_tPrevBtnRect.Left, m_tPrevBtnRect.Top, m_tPrevBtnRect.Right, m_tPrevBtnRect.Bottom, .AlphaBlend(vbBlack, vbHighlight, 60), .AlphaBlend(vbBlack, vbHighlight, 60)
                    End If
                End If
                tT.Top = tT.Top + 2
                tT.Bottom = tT.Top + 9
                ...
                If Not pIsPrevButtonEnabled Then
                    ' Draw empty arrows
                    For i = 4 To 0 Step -1
                        .DrawLine tT.Right - i, tT.Top + i, tT.Right - i, tT.Bottom - i, .AlphaBlend(vbHighlight, vbGrayText, 90)
                    Next
                End If
            End If
            '== END PREV, NEXT AND CLOSE BUTTONS
            If m_NavigationStyle = ensDropDownButton Then
                ...
                If m_lSelBtn = 4 Then
                    ' If it is pressed
                    If m_lPressedBtn = 4 Then
                        .FillRect m_tDropBtnRect.Left, m_tDropBtnRect.Top, m_tDropBtnRect.Right, m_tDropBtnRect.Bottom, .AlphaBlend(vbBlack, m_lClrSelButtons, 40)
                        .Draw3DRect m_tDropBtnRect.Left, m_tDropBtnRect.Top, m_tDropBtnRect.Right, m_tDropBtnRect.Bottom, .AlphaBlend(vbBlack, vbHighlight, 120), .AlphaBlend(vbBlack, vbHighlight, 120)
                        ' If it is selected
                    Else
                        .FillRect m_tDropBtnRect.Left, m_tDropBtnRect.Top, m_tDropBtnRect.Right, m_tDropBtnRect.Bottom, m_lClrSelButtons
                        .Draw3DRect m_tDropBtnRect.Left, m_tDropBtnRect.Top, m_tDropBtnRect.Right, m_tDropBtnRect.Bottom, .AlphaBlend(vbBlack, vbHighlight, 60), .AlphaBlend(vbBlack, vbHighlight, 60)
                    End If
                End If
                ' If it is pressed
                If m_lPressedBtn = 4 Then
            ...
            ' END Dropdown button
            ' Draw topmost border
            .DrawLine tR.Left, tR.Top, tR.Right, tR.Top, m_lClrBorder
            ' draw right line
            .DrawLine tR.Right - 1, tR.Top, tR.Right - 1, tR.Bottom, m_lClrBorder
            '============================================================
        End With
    Else
        With m_oMemDC
            ...
            .Cls vbApplicationWorkspace
            ' Draw LEFT control border
            .DrawLine 0, 0, 0, m_lTabHeight, m_lClrBorder
            .DrawLine 1, 0, 1, m_lTabHeight, .AlphaBlend(m_lClrback, vbWhite, 100)
            .DrawLine 2, 0, 2, m_lTabHeight, vbButtonFace
            m_oMemDCTabs.DrawLine 0, 0, 0, m_lTabHeight, vbButtonFace
            ' Draw RIGHT control border
            .DrawLine tR.Right - 1, 0, tR.Right - 1, m_lTabHeight, m_lClrBorder
            .DrawLine tR.Right - 2, 0, tR.Right - 2, m_lTabHeight, .AlphaBlend(m_lClrback, vbWhite, 100)
            .DrawLine tR.Right - 3, 0, tR.Right - 3, m_lTabHeight, vbButtonFace
        End With
        m_lLastSelMDIChild = 0
    End If
...
Private Sub DrawTabsdotNET(ByVal lHwnd As Long)
...
Dim bIntersect As Long
    ...
    With m_oMemDC
        ...
        ' Draw bar highlight line
        .DrawLine tR.Left + 3, m_lTabHeight - 3, tR.Right - 3, m_lTabHeight - 3, m_lClrTabBorder
        .DrawLine tR.Left + 3, m_lTabHeight - 1, tR.Right - 3, m_lTabHeight - 1, vbButtonFace
        .DrawLine tR.Left + 3, m_lTabHeight - 2, tR.Right - 3, m_lTabHeight - 2, m_lClrBorder
        '== END BORDERS
        '============================================================
    End With
    ...
    If m_hWndChild.Count > 0 Then
        With m_oMemDCTabs
            ...
            lTabLeft = 4
            ReDim m_tTabR(1 To m_hWndChild.Count) As RECT
            ReDim m_tXRectOnEachTab(1 To m_hWndChild.Count) As RECT
            ' For each MDI child window
            For Each hWndChild In m_hWndChild
                i = i + 1
                ' Get MDI child caption
            ...
            'Background line for .NET style 2
            If m_eStyle = mtsdotNET_2 Then
                .DrawLine 0, m_tTabR(i).Bottom - 1, tR.Right, m_tTabR(i).Bottom - 1, m_lClrInnerBorder
            End If
            i = 0
            ' For each MDI child window
            For Each hWndChild In m_hWndChild
                ...
                If bIntersect Then
                    ...
                    If lSelMDIChild = hWndChild Then
                        ...
                        If m_eStyle = mtsdotNET_1 Then
                            ...
                            .DrawLine m_tTabR(i).Right - 1, m_tTabR(i).Top + 1, m_tTabR(i).Right - 1, m_tTabR(i).Bottom, .TranslateColor(vbApplicationWorkspace)
                        ElseIf m_eStyle = mtsdotNET_2 Then
                            ' Draw tab background
                            .FillRect m_tTabR(i).Left, m_tTabR(i).Top, m_tTabR(i).Right, m_tTabR(i).Bottom, m_lClrTabBack1
                            ' Draw tab border
                            .DrawLine m_tTabR(i).Left, m_tTabR(i).Top, m_tTabR(i).Left, m_tTabR(i).Bottom + 5, .TranslateColor(vbButtonShadow)
                            .DrawLine m_tTabR(i).Left, m_tTabR(i).Top, m_tTabR(i).Right, m_tTabR(i).Top, .TranslateColor(vbButtonShadow)
                            .DrawLine m_tTabR(i).Right - 1, m_tTabR(i).Top + 1, m_tTabR(i).Right - 1, m_tTabR(i).Bottom, .TranslateColor(vbButtonShadow)
                            .Draw3DRect m_tXRectOnEachTab(i).Left, m_tXRectOnEachTab(i).Top, m_tXRectOnEachTab(i).Right, m_tXRectOnEachTab(i).Bottom, vbBlack, vbBlack
                        End If
                    ...
                    Else
                        ...
                        If m_eStyle = mtsdotNET_2 Then
                            ' repair broken lines (lol)
                            .DrawLine m_tTabR(i).Left, m_tTabR(i).Bottom - 1, m_tTabR(m_hWndChild.Count).Right + 10, m_tTabR(i).Bottom - 1, m_lClrInnerBorder
                        End If
                    End If
                    ' Draw tab caption
                    lTabTop = m_tTabR(i).Top + (((m_tTabR(i).Bottom - m_tTabR(i).Top) / 2) - (.TextHeight(m_TabCaption) / 2))
        ...
        With m_oMemDC
            ...
            ' line under the next, prev and close btns
            If m_eStyle = mtsdotNET_2 Then
            .DrawLine tT.Left, tT.Bottom - 1, tT.Right - 2, tT.Bottom - 1, m_lClrOuterBorder
            End If
            ...
            If m_lSelBtn = 3 Then
                ' If it is pressed
                If m_lPressedBtn = 3 Then
                    .Draw3DRect m_tCloseBtnRect.Left, m_tCloseBtnRect.Top, m_tCloseBtnRect.Right, m_tCloseBtnRect.Bottom, vbButtonShadow, m_lClrButtonBorder
                    ' If it is selected
                Else
                    .Draw3DRect m_tCloseBtnRect.Left, m_tCloseBtnRect.Top, m_tCloseBtnRect.Right, m_tCloseBtnRect.Bottom, m_lClrButtonBorder, vbButtonShadow
                End If
            End If
            ' If it is pressed
            If m_lPressedBtn = 3 Then
            ...
            If m_NavigationStyle = ensScrollButtons Then
                ...
                If m_lSelBtn = 2 Then
                    ' If it is pressed
                    If m_lPressedBtn = 2 Then
                        .Draw3DRect m_tNextBtnRect.Left, m_tNextBtnRect.Top, m_tNextBtnRect.Right, m_tNextBtnRect.Bottom, vbButtonShadow, m_lClrButtonBorder
                        ' If it is selected
                    Else
                        .Draw3DRect m_tNextBtnRect.Left, m_tNextBtnRect.Top, m_tNextBtnRect.Right, m_tNextBtnRect.Bottom, m_lClrButtonBorder, vbButtonShadow
                    End If
                End If
                tT.Left = tT.Left + 4
                tT.Top = tT.Top + 2
            ...
            If m_NavigationStyle = ensScrollButtons Then
                ...
                If m_lSelBtn = 1 Then
                    ' If it is pressed
                    If m_lPressedBtn = 1 Then
                        .Draw3DRect m_tPrevBtnRect.Left, m_tPrevBtnRect.Top, m_tPrevBtnRect.Right, m_tPrevBtnRect.Bottom, vbButtonShadow, m_lClrButtonBorder
                        ' If it is selected
                    Else
                        .Draw3DRect m_tPrevBtnRect.Left, m_tPrevBtnRect.Top, m_tPrevBtnRect.Right, m_tPrevBtnRect.Bottom, m_lClrButtonBorder, vbButtonShadow
                    End If
                End If
                tT.Top = tT.Top + 2
                tT.Bottom = tT.Top + 9
            ...
            If m_NavigationStyle = ensDropDownButton Then
                ...
                End If
            End If
            ' --draw a border line under the three buttons
            If m_eStyle = mtsdotNET_2 Then
                .DrawLine m_tDropBtnRect.Left, m_tCloseBtnRect.Bottom + 2, m_tCloseBtnRect.Right + 3, m_tCloseBtnRect.Bottom + 2, m_lClrInnerBorder
                .DrawLine m_tTabR(m_hWndChild.Count).Right - 1, m_tDropBtnRect.Bottom + 2, m_tDropBtnRect.Left, m_tCloseBtnRect.Bottom + 2, m_lClrInnerBorder
            End If
            '============================================================
        End With
    Else
...
Private Sub GetMDIChildWindows()

'********************************************************************
'* Name: GetMDIChildWindows
...
Dim tPA As POINTAPI
    ...
    If tPA.x >= m_tCloseBtnRect.Left And tPA.x <= m_tCloseBtnRect.Right And tPA.y >= m_tCloseBtnRect.Top And tPA.y <= m_tCloseBtnRect.Bottom Then
        bInButton = 3: pIsWithinTab = -1: Exit Function
    End If
    If m_NavigationStyle = ensScrollButtons Then
        If tPA.x >= m_tNextBtnRect.Left And tPA.x <= m_tNextBtnRect.Right And tPA.y >= m_tNextBtnRect.Top And tPA.y <= m_tNextBtnRect.Bottom Then
            'If pIsNextButtonEnabled Then bInButton = 2':( --> replaced by:
        ...
        If tPA.x >= m_tPrevBtnRect.Left And tPA.x <= m_tPrevBtnRect.Right And tPA.y >= m_tPrevBtnRect.Top And tPA.y <= m_tPrevBtnRect.Bottom Then
            'If pIsPrevButtonEnabled Then bInButton = 1':( --> replaced by:
            If pIsPrevButtonEnabled Then
                bInButton = 1
            End If
            pIsWithinTab = -1
        ...
        End If
    Else
        If tPA.x >= m_tDropBtnRect.Left And tPA.x <= m_tDropBtnRect.Right And tPA.y >= m_tDropBtnRect.Top And tPA.y <= m_tDropBtnRect.Bottom Then
            'If pIsNextButtonEnabled Then bInButton = 2':( --> replaced by:
            bInButton = 4
            pIsWithinTab = -1
            Exit Function
...
Dim tR As RECT
    ...
    If m_hWndChild.Count > 0 Then
        ...
        If (lMaxRight > lSize) Then
            ...
            'If (lMaxRight - m_lOffsetX < lSize) Then m_lOffsetX = lMaxRight - lSize':( --> replaced by:
            If (lMaxRight - m_lOffsetX < lSize) Then
                m_lOffsetX = lMaxRight - lSize
            End If
...
'********************************************************************
    ...
    With m_oMemDC
        ...
        ElseIf m_eStyle = mstOneNote Then
            m_lClrTabSeparator = vbButtonShadow
            
        ElseIf m_eStyle = mtsdotNET_1 Or m_eStyle = mtsdotNET_2 Then
            m_lClrback = .ShiftColor(vb3DFace, 24)
            m_lClrTabBack1 = vbButtonFace
        ...
        ElseIf m_eStyle = mtsOffice2003 Then
            If m_sCurrentSystemThemename = "Classic" Then
                m_lClrback = .AlphaBlend(vbButtonShadow, vbWhite, 160)
                m_lClrTabBack1 = .AlphaBlend(vbButtonShadow, vbWhite, 85)
                m_lClrTabBack2 = .AlphaBlend(vbButtonShadow, vbButtonFace, 230)
                m_lClrButtonBorder = .AlphaBlend(vbButtonShadow, &HE0E0E0, 140)
                m_lClrTabSeparator = .AlphaBlend(vbButtonShadow, vbGrayText, 120)
                m_lClrBorder = .AlphaBlend(vbButtonShadow, vbBlack, 190)
                m_lClrOuterBorder = .AlphaBlend(m_lClrBorder, vbWhite, 210)
                m_lClrInnerBorder = .AlphaBlend(m_lClrBorder, vbWhite, 100)
                m_lClrTabInactiveFore = .AlphaBlend(vbGrayText, vbBlack, 130)
                m_lClrSelButtons = .AlphaBlend(vbHighlight, .AlphaBlend(vbWindowBackground, vbButtonFace, 220), 85)
            Else
                m_lClrback = .AlphaBlend(vbHighlight, .AlphaBlend(vbButtonFace, vbWhite, 100), 145)
                m_lClrTabBack1 = .AlphaBlend(vbHighlight, vbWhite, 90)
                m_lClrTabBack2 = .AlphaBlend(vbHighlight, vbButtonShadow, 210)
                m_lClrButtonBorder = .AlphaBlend(vbHighlight, &HE0E0E0, 140)
                m_lClrTabSeparator = .AlphaBlend(vbHighlight, vbGrayText, 120)
                m_lClrBorder = .AlphaBlend(vbHighlight, vbBlack, 190)
                m_lClrOuterBorder = .AlphaBlend(m_lClrBorder, vbWhite, 210)
                m_lClrInnerBorder = .AlphaBlend(m_lClrBorder, vbWhite, 100)
                m_lClrTabInactiveFore = .AlphaBlend(vbGrayText, vbBlack, 130)
                m_lClrSelButtons = &HC2EEFF
            End If
        End If
    End With
...
'Private Function pIsMouseInXButton() As Boolean
...
'    tPA.x = tPA.x + 2  '2=border
'
'    ' Check if mouse is in Close(X) Button on each tab
'    For i = 1 To m_hWndChild.Count
'        With m_tXRectOnEachTab(i)
'            If PtInRect(m_tXRectOnEachTab(i), tPA.x, tPA.y) Then
'                ' We are inside the X Button of i tab
'                m_lMouseOverXButton = i
'                m_oTimer.Enabled = True
'            End If
'        End With
'    Next
'    Debug.Print m_lMouseOverXButton
'
'End Function
...
Dim tOVI As OSVERSIONINFO
    ...
    If tOVI.dwMajorVersion > 5 And tOVI.dwMinorVersion >= 1 Then
        ...
        If Not lTheme = 0 Then
            ...
            'If lpOS > 1 Then m_sThemeName = Left$(m_sThemeName, lpOS - 1)':( --> replaced by:
            If lpOS > 1 Then
                m_sThemeName = Left$(m_sThemeName, lpOS - 1)
            End If
...
Private Function pMenuAdd()
    Load mnuList(m_hWndChild.Count)
End Function

Private Function pMenuRem()
                ...
                If m_lDraggingTab = 0 Then
                    ...
                    If iMouseBtn = vbRightButton Then
                        ...
                        If lTab > 0 Then
                            ...
                            Select Case m_eStyle
                            Case mtsdotNET_1, mtsdotNET_2, mtsOffice2003, mtsOffice2000, mtsWhidbey
                                lColor = vbButtonFace
                            Case mstOneNote
                                If m_hWndChild.Count > 0 Then
                                    lColor = m_lColorTable(m_oColorIndex(m_hWndChild.Count))
                                Else
                                    lColor = m_lColorTable(1)
                                End If
                            End Select
    ...
    Case WM_LBUTTONDOWN
        ...
        If lBtn > 0 Then
            ...
            Case 2
                ...
                End If
            Case 4
                ' Dropdown Menu (same as WindowList)
            End Select
            pDrawControl m_lMDIClient
        End If
    Case WM_LBUTTONUP, WM_RBUTTONUP
        ...
        If m_lPressedBtn > 0 Then
            ...
            If lBtn = 3 Then
                ...
                ' Added by NR End
            ElseIf lBtn = 4 Then
                ' --Drop Down Button Pressed and released
                ' Get MDI client window dc
                lChDC = GetWindowDC(m_lMDIClient)
                ' Get MDI client dimensions
                GetWindowRect m_lMDIClient, dR
                ' Set the popupmenu X,Y vals
                dR.Left = dR.Right - 33
                dR.Top = dR.Top + m_tCloseBtnRect.Bottom + 1
                RaiseEvent DropdownButtonClick '(dR.Left, dR.Top)
            End If
            ReleaseCapture
            pDrawControl m_lMDIClient
        End If
...
Private Sub m_oTimer_Timer()
    ...
    ElseIf m_lSelBtn = 2 And pIsNextButtonEnabled Then
        ...
        pDrawControl m_lMDIClient
    End If
    'If Not pIsMouseInXButton Then
    '    m_oTimer.Enabled = False
    '    m_lMouseOverXButton = -1
    '    pDrawControl m_lMDIClient
    'End If
...
Dim lStyle As Long
    ...
    With PropBag
        m_eStyle = .ReadProperty("Style", mtsWhidbey)
        Set m_fFont = .ReadProperty("Font", UserControl.Font)
        m_bDrawFocusRect = .ReadProperty("ShowFocusRect", False)
        m_bDrawIcons = .ReadProperty("DrawIcons", True)
        m_NavigationStyle = .ReadProperty("NavigationStyle", ensScrollButtons)
    End With
    ...
    If UserControl.Ambient.UserMode Then
        ...
        If m_lMDIClient <> 0 Then
            ...
            With m_oParentSubclass
                ...
                .AddBeforeMsgs WM_ACTIVATE, WM_SIZING
            End With
        End If
    End If
...
Attribute DrawIcons.VB_Description = "Returns/Sets a value to determine whether to draw icons on each tab."
Attribute DrawIcons.VB_ProcData.VB_Invoke_Property = ";Appearance"
...
Public Property Get NavigationStyle() As enumNavigationStyle

    NavigationStyle = m_NavigationStyle
    
End Property

Public Property Let NavigationStyle(ByVal NewStyle As enumNavigationStyle)
    
    m_NavigationStyle = NewStyle
    PropertyChanged "NavigationStyle"
    pDrawControl m_lMDIClient
...
Attribute ShowFocusRect.VB_ProcData.VB_Invoke_Property = ";Appearance"
    ...
    ShowFocusRect = m_bDrawFocusRect

End Property

Public Property Let ShowFocusRect(ByVal Value As Boolean)
...
Attribute Style.VB_Description = "Returns/sets a value to determine the style used to draw the tabs."
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    With PropBag
        .WriteProperty "Style", m_eStyle, mtsWhidbey
        .WriteProperty "Font", m_fFont, UserControl.Font
        .WriteProperty "ShowFocusRect", m_bDrawFocusRect, False
        .WriteProperty "DrawIcons", m_bDrawIcons, True
        .WriteProperty "NavigationStyle", m_NavigationStyle, ensScrollButtons
    End With
