...
'   Appearance Costants
Public Enum ucAppearanceConstants
    [Flat] = &H0
    [3D] = &H1
End Enum
...
Private m_Appearance        As ucAppearanceConstants
...
Public Property Get Appearance() As ucAppearanceConstants
    Appearance = m_Appearance
End Property

Public Property Let Appearance(lNewValue As ucAppearanceConstants)
    '   Store the Value
...
Private Function ButtonAppearance(cmdButton As CommandButton, lButtonStyle As ucAppearanceConstants)
...
Private Sub cmdPick_Click()
    ...
    With UserControl
        ...
        UserControl.cmdDrop.Visible = False
            ...
            Case [ucOpen], [ucSave]
                ...
                If (psFile.bCanceled = False) And (psFile.nFilesSelected > 0) Then
                    If m_DialogType = [ucOpen] Then
                        ...
                        UserControl.cmdDrop.Visible = m_MultiSelect
                        ...
                        If m_MultiSelect Then
                            ...
                            Else
                                ...
                                For i = 1 To m_FileCount
                                    .cmbMultiSel.AddItem TrimPathByLen(m_Path & psFile.sFiles(i), .txtResult.Width - 40)
...
Retry:
                            ...
                            If Len(sExt) = 0 Then
                                If MsgBox("     The File Extension is Invalid!" & vbCrLf & vbCrLf & "File will be saved with " & Chr$(34) & ".txt" & Chr$(34) & " extension.", vbExclamation + vbOKCancel, "ucPickBox") = vbOK Then
                                    ...
                                    sExt = ".txt"
                                Else
                                    '   Give them anther try to get this right...
                                    GoTo Retry
                                End If
                            End If
...
Public Property Let Path(sNewPath As String)
    ...
    DialogMsg(m_DialogType) = QualifyPath(TrimPathByLen(m_Path, UserControl.txtResult.Width - UserControl.cmdPick.Width - 40))
    PropertyChanged "Path"
End Property
    ...
    If iTextWidth < iInputLen% Then
        ...
        If InStr(1, sInput$, "\") > 0 Then 'LOCAL
            ...
            If UBound(aBuffer()) > LBound(aBuffer()) Then
                ...
                If (Printer.TextWidth(sBeginning$) + Printer.TextWidth(sReplaceString$) + Printer.TextWidth(sEnd$)) > iTextWidth% Then
    ...
    With UserControl
        Select Case m_DialogType
            ...
            Case [ucOpen], [ucSave]
                ...
                If .txtResult.Text = m_DialogMsg(i) Then Exit Sub
...
Private Sub UserControl_Show()
    With UserControl
        If m_UseDialogText Then
            .txtResult.Text = m_DialogMsg(m_DialogType)
        Else
            .txtResult.Text = ""
        End If
    End With
